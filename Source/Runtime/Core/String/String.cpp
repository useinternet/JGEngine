...
PString PString::ToUpper(const PString& inStr)
{
	HRawString rawStr = inStr.GetRawString();
	std::transform(inStr.GetRawString().cbegin(), inStr.GetRawString().cend(), rawStr.begin(), ::toupper);
	return PString(rawStr.c_str());
}

PString PString::ToLower(const PString& inStr)
{
	HRawString rawStr = inStr.GetRawString();
	std::transform(inStr.GetRawString().cbegin(), inStr.GetRawString().cend(), rawStr.begin(), ::tolower);
	return PString(rawStr.c_str());
}
