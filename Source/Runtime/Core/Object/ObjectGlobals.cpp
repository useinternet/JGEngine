...
PName JGMeta::GetMetaValue(const PName& Key) const
{
	HHashSet<PName> valueSet;
	if (GetMetaValues(Key, valueSet) == false)
	{
		return NAME_NONE;
	}
...
int32 JGEnum::GetValueByEnumName(PName enumName) const
{
	int32 index = GetIndexByEnumName(enumName);
	return GetIndexByValue(index);
}

int32 JGEnum::GetIndexByEnumName(PName enumName) const
{
	int32 index = 0;
	for (PName name : ElementNames)
	{
		if (name == enumName)
		{
			return index;
